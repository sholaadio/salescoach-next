...
export const accent = (role: string) => ({ ceo: "#F59E0B", gm: "#3B82F6", head_sales: "#10B981", head_creative: "#8B5CF6", hr: "#EC4899", teamlead: "#F97316", closer: "#5E6AD2" }[role] ?? "#3B82F6");
...
export const filterByPeriod = <T extends Record<string, unknown>>(items: T[], field: keyof T, period: string): T[] => {
  const now = new Date();
...
export const calcSalesCommission = (assigned: number, delivered: number, upsells = 0, repeats = 0, referrals = 0) => {
  ...
  const rate = d / a * 100; let perOrder = 0, tier = "none";
  ...
  const upsellBonus = upsellLocked ? 0 : (+upsells ||
